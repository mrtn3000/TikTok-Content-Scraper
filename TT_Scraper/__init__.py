...
class TT_Scraper(HTML_Scraper):
    ...
    from ._download_data import _download_data, write_video, write_pictures, write_metadata_package, write_slide_audio, write_cover
    from ._exception_handler import _exception_handler
    from ._fetch_custom_tdk import _fetch_custom_tdk
    ...
    def scrape(self, id, scrape_content : bool = False, download_metadata = True, download_content = True):
        ...
        try:
            ...
            # Fetch custom TDK data (AI summary)
            ai_summary = self._fetch_custom_tdk(id)
            if ai_summary:
                metadata_package["video_metadata"]["ai_summary"] = ai_summary
            ...
            # extract cover URL
            try:
                cover_url = rehydration_data_json["__DEFAULT_SCOPE__"]['webapp.video-detail']['itemInfo']['itemStruct']["video"]["cover"]
        ...
        if binary_video:
            content_binary = {"type": "video", "mp4_binary": binary_video, "cover_url": cover_url}
        elif binaries_slides:
            content_binary = {"type": "slide", "slide_pictures": binaries_slides, "slide_audio": binary_slides_audio, "cover_url": cover_url}
        else:
            content_binary = None
